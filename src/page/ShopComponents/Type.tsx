import React from "react";

const Type = () => {
  return (
    <div className="section">
      <section className="bbd-component bbd-full-width-container">
        <div className="outer-container">
          <div className="content-container">
            <div>
              <div className="beatsRTE parbase section">
                <section className="bbd-component bbd-rte-container">
                  <div className="rte-container-ae8485ca-9148-37a9-b7cd-39bfcfa65a21 beatsRTE-desktoptext component-section-spacing beatsRTE">
                    <div className="font-updated e-full h-[104px] bg-[#333333]">
                      <ul className="flex justify-center items-center h-full text-inverse gap-[56px] pt-[2px] text-center">
                        <li className="w-[70px] rounded-[30px] flex flex-col items-center text-[-.75em] gap-y-[2px]">
                          <a
                            href="#earbuds"
                            rel="me noopener noreferrer"
                            className="text-inverse"
                            data-analytics-click="true"
                            data-analytics-link-type="siteClick"
                            data-analytics-label="PLP|CategoryJumper|Earbuds"
                            data-aria-label-prefix="Jump to Earbuds"
                          >
                            <div className="flex justify-center items-center">
                              <svg
                                width="40"
                                height="40"
                                version="1.1"
                                id="Layer_1"
                                xmlns="http://www.w3.org/2000/svg"
                                xmlnsXlink="http://www.w3.org/1999/xlink"
                                x={0}
                                y={0}
                                viewBox="0 0 40 40"
                                xmlSpace="preserve"
                              >
                                <g>
                                  <path
                                    className="st0"
                                    d="M8.9,27.4c-0.6-1.3-1.1-2.3-1.4-3.1c-0.5,1.3-0.8,2.8-0.4,4.6c0.8,3,3.1,5.1,6.1,5.5c-0.4-0.4-0.8-0.9-1.2-1.5
      C12,32.9,10.9,31.4,8.9,27.4z"
                                  ></path>
                                  <path
                                    className="st0"
                                    d="M18.1,29.4c0,0-0.1-0.1-0.1-0.1c-0.2-0.2-0.5-0.3-0.8-0.3c-0.3,0-0.5,0.1-0.8,0.3c0,0-0.1,0.1-0.1,0.1
      c-0.2,0.2-0.3,0.5-0.4,0.8c0,0,0,0.1,0,0.1c0,0.1,0,0.1,0,0.2c0,0.2,0.1,0.4,0.2,0.5c0,0.1,0.1,0.1,0.1,0.2
      c0.1,0.1,0.2,0.2,0.3,0.2c0.1,0,0.2,0.1,0.3,0.1c0.1,0,0.2,0.1,0.3,0.1c0.1,0,0.2,0,0.3-0.1c0.1,0,0.3-0.1,0.4-0.2
      c0.1-0.1,0.2-0.1,0.2-0.2c0.2-0.2,0.4-0.6,0.4-0.9c0-0.3-0.1-0.5-0.3-0.7C18.2,29.5,18.2,29.5,18.1,29.4z"
                                  ></path>
                                  <path
                                    className="st0"
                                    d="M21.6,28.1l-4.3-7.5c-0.6-1-1.5-1.6-3.1-2.5c-0.7-0.3-1.7-1.3-1.3-3c0.8-3.2,2.9-5,4.9-6.3
      C19,8,19.1,7.3,19.1,6.6c0-0.4-0.1-1-0.1-1.3C19,5.1,18.9,5,18.8,5c-0.1-0.1-0.2-0.1-0.4-0.1C18.2,4.9,18,5,17.8,5
      C7.1,9.4,7.3,17.9,7.6,20.4c0.2,1.8,0.4,3,2.2,6.5c1.9,3.9,3.1,5.3,3.1,5.4c1.8,3,4.7,3.3,6.7,2.2C21.5,33.5,23,31,21.6,28.1z
      M19.2,31.1c-0.3,0.8-1.1,1.4-2,1.4c-0.7,0-1.3-0.3-1.7-0.8c-0.2-0.3-0.3-0.6-0.4-0.9c-0.1-0.3-0.1-0.6-0.1-1v-3.1
      c0.3-0.2,0.6-0.3,0.9-0.4l0,2.4c0.4-0.3,0.9-0.5,1.4-0.5c0.5,0,1,0.2,1.3,0.5C19.3,29.3,19.5,30.3,19.2,31.1z"
                                  ></path>
                                  <path
                                    className="st0"
                                    d="M33.1,25.3c-0.1-1.8-0.9-3.4-2.4-4.4c-1.6-1.1-3.7-1.4-5.6-1c0.7,0.6,1.2,1.3,1.6,2c0,0,0.6,1.2,0.8,2.3
      c0.7,3.4-0.4,6-1.5,7.4c-0.1,0.1-0.2,0.2-0.3,0.4c1.7,0.2,3.6-0.3,5.1-1.3c1-0.7,2.2-2,2.3-4.1C33.1,25.9,33.1,25.3,33.1,25.3z"
                                  ></path>
                                  <path
                                    className="st0"
                                    d="M26.5,24.4c-0.2-1-0.7-2-0.8-2c-1-2-3.7-3.8-6-4.1c-1.3-0.1-2.6-0.1-3.6-0.1c0.8,0.5,1.5,1.1,2,1.9l4.4,7.5
      c1.1,2.3,0.7,4.5-0.5,6c1.2-0.6,2.3-1.5,3.2-2.6C26.1,29.7,27.1,27.5,26.5,24.4z"
                                  ></path>
                                </g>
                              </svg>
                            </div>
                            Earbuds
                          </a>
                        </li>
                        <li className="w-[70px] rounded-[30px] flex flex-col gap-4 items-center text-[-.75em] gap-y-[2px]">
                          <a
                            href="#headphones"
                            rel="me noopener noreferrer"
                            className="text-inverse"
                            data-analytics-click="true"
                            data-analytics-link-type="siteClick"
                            data-analytics-label="PLP|CategoryJumper|Headphones"
                            data-aria-label-prefix="Jump to Headphones"
                          >
                            <div className="flex justify-center items-center">
                              <svg
                                width="40"
                                height="40"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <g clipPath="url(#a)" fill="#fff">
                                  <path d="M23.34 19.67c.043.126.093.25.15.37.06.13.13.26.2.42.143.271.3.535.47.79.18.28.38.57.58.85l.33.47c.1.15.2.29.28.42.395.585.701 1.226.91 1.9a6.94 6.94 0 0 1 .31 2.11 7.05 7.05 0 0 1-14.1 0 7.267 7.267 0 0 1 .31-2.08c.2-.682.504-1.33.9-1.92l.3-.45.32-.47c.2-.28.41-.57.59-.85.167-.255.32-.519.46-.79l.2-.42.16-.37A8.25 8.25 0 1 0 27.77 27a8.251 8.251 0 0 0-4.43-7.31v-.02z"></path>
                                  <path d="M24.65 23.47c-.26-.39-.58-.84-.9-1.3a11.292 11.292 0 0 1-.82-1.34 6.549 6.549 0 0 1-.47-1.12 5.886 5.886 0 0 1-.16-1.51V5.8c0-1.83-.07-3.67-.14-3.82A12.32 12.32 0 0 0 17 2a33.619 33.619 0 0 0-.14 3.82V18.22a5.41 5.41 0 0 1-.17 1.51c-.113.39-.27.766-.47 1.12-.238.467-.512.915-.82 1.34-.31.46-.64.91-.89 1.3a6.06 6.06 0 0 0-.79 1.64 6.001 6.001 0 0 0-.4 1.87 6.2 6.2 0 1 0 12.4 0 6.37 6.37 0 0 0-1.07-3.53zm-5.13 7.76A4.27 4.27 0 0 1 15.27 27a4.19 4.19 0 0 1 2.11-3.65V26.92c.006.165.026.329.06.49.03.16.08.314.15.46a2.111 2.111 0 0 0 1 1.03 2 2 0 0 0 .94.22 2.13 2.13 0 0 0 1.98-1.38 2.07 2.07 0 0 0 .08-1.25c-.084-.417-.29-.8-.59-1.1a2.068 2.068 0 0 0-.66-.39 2 2 0 0 0-.72-.16c-.248.012-.491.066-.72.16a1.91 1.91 0 0 0-.64.38v-2.4c.2-.065.403-.115.61-.15.214-.04.432-.06.65-.06a4.25 4.25 0 1 1 0 8.5v-.04zM18.27 27a1.27 1.27 0 0 1 2.17-.91 1.239 1.239 0 0 1 .38.9 1.27 1.27 0 0 1-.37.9 1.24 1.24 0 0 1-.9.38A1.28 1.28 0 0 1 18.27 27z"></path>
                                </g>
                                <defs>
                                  <clipPath id="a">
                                    <path
                                      fill="#fff"
                                      transform="translate(11 1)"
                                      d="M0 0h17v35H0z"
                                    ></path>
                                  </clipPath>
                                </defs>
                              </svg>
                            </div>
                            Headphones
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                </section>
              </div>
              <div className="genericSpacing parbase section"></div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Type;
